...
## 1.1 Introduction
In the first part of this lab, using provided data, we analyzed the  reliability of a software and its failure rate over time to determine appropriate models for representing and predicting failures of the system. The failure dataset consisted of interval-based failure observations extracted from the provided ZIP file. The data was imported into C-SFRAT following the required format, with failure occurrences mapped to their corresponding test intervals and execution time.
...
## 1.3 Range Analysis
Range analysis was performed by gradually increasing the number of failure data points used in the model fitting (from 0 to 31 intervals). In the early stages of the dataset, the evaluation metrics (AIC, BIC, SSE) showed high variability and model rankings were inconsistent, indicating that the failure process had not yet reached a stable reliability growth phase. This behavior is consistent with an unstable region in a Laplace trend interpretation, where early data does not yet reliably reflect system behavior. As additional data points were included, the metrics gradually stabilized and the same top-performing models (DW3 (F) and IFRGSB (F)) consistently appeared in the rankings. The variation in AIC, BIC, and SSE became minimal, and ranking changes ceased after approximately interval 25, indicating convergence of model behavior. Additionally, the reliability and failure intensity curves became smoother and more representative of steady reliability growth. Based on this stabilization in both statistical metrics and trend behavior, the later portion of the dataset (after approximately 25 intervals) was selected as the valid modeling region, as it provides more reliable and stable conditions for reliability growth analysis and model selection.
...
Although C-SFRAT does not directly output a dedicated reliability function plot, system reliability can be derived from the Mean Value Function (MVF) produced by the reliability growth models. The MVF represents the expected cumulative number of failures over time, and it can be used to infer the probability that the system has not experienced a failure up to a given time.
